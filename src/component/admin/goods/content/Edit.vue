    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="所属类别" prop="category_id">
        ...
        <el-select v-model="ruleForm.category_id" placeholder="请选择活动区域">
          <!-- goodsCategory是请求接口获取到的分类列表,将来用户要选择其中一个分类的ID传给后台 -->
      ...
      <el-form-item label="是否发布">
        <el-switch v-model="ruleForm.starus"></el-switch>
      </el-form-item>
      ...
      <el-form-item label="上传封面" prop="imgList">
        ...
        <el-upload class="upload-demo" action="http://157.122.54.189:9095/admin/article/uploadimg" :on-remove="imgRemove" :on-success="imgUploaded" :file-list="ruleForm.imgList" list-type="picture">
...
export default {
    ...
    methods: {
        ...
        goodsModify() {
            this.$http
                .post(this.$api.gsEdit + this.id, this.ruleForm)
                .then(res => {
        ...
        getGoods() {
            this.$http.get(this.$api.gsDetail + this.id).then(res => {
                ...
                // 默认是个字符串,给它转换成number类型,这样它就会自动和分类option的value进行匹配
        ...
        // 1 删除封面图
        // 2 然后遍历this.ruleForm.imgLis,从中删除掉与之相同url的图片数据
    ...
    // 组件初始化完毕后,先拿到url中的id,再请求接口获取商品数据进行回显
    created() {
